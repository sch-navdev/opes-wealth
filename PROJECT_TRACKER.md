...
- `src/components/profile-form.tsx` — Client Component. Email shown with a green "Verified" `Badge` (`isEmailVerified`) or a "Send Verification Link" button wired to `resendEmailVerification`. Avatar uploader: a shadcn `Avatar` (image or initials fallback) plus a hidden `<input type="file" accept="image/*">`; on selection, `FileReader.readAsDataURL()` converts the file to Base64, stored in state and mirrored into a hidden form input (`avatar_base64`) so it submits with the rest of the form. First/Last Name row, Phone Number + an outlined "Verify Phone" button (`alert('SMS verification integration coming soon.')` — a stub, no real SMS integration yet), and a "Residential Address" grid (Street, PO Box, City, Postal Code, Landmark, Country). Submission uses the same client-driven `startTransition` + `FormData` pattern as `AddAssetDialog`/`LoginForm` (kept consistent with the rest of the app). The `form` shadcn component and its `react-hook-form`/`@hookform/resolvers`/`zod` dependencies, installed alongside it but never adopted, were removed in the codebase audit pass below.
...
- **Robustness/accessibility fixes**: `dashboard/settings/page.tsx` no longer uses a `user.email!` non-null assertion (redirects to `/login` if email is ever absent instead). `dashboard/page.tsx`'s liability-row class list now uses `cn()` instead of string concatenation. `mfa.ts`'s `currentAuthenticationMethods` check is now properly typed and no longer includes a speculative `"passkey"` method value that never actually occurs (Supabase only emits `"webauthn"`). `profile-form.tsx`: the address "Country" combobox now has a properly associated `<Label htmlFor>` (via a new `id` prop on `CountryCombobox`), and the phone-verification stub now shows inline status text instead of firing a native `alert()`. Two-column grids in `profile-form.tsx` (name row, address grid) now stack on narrow screens (`grid-cols-1 sm:grid-cols-2`); the dashboard header now wraps instead of forcing a single row.
